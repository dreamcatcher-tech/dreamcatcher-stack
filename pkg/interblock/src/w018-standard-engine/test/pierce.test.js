...
describe('pierce', () => {
  ...
  test('multiple simultaneous pings maintain order', async () => {
    ...
    // buffer the increase requests so db can settle
    const buffer = []
    const increase = sqsIncrease.pushDirect
    sqsIncrease.pushDirect = (action) => buffer.push(action)
    ...
    // unload the buffer
    const rxdb = await base.getRxdb()
    await rxdb.requestIdlePromise() // await db
